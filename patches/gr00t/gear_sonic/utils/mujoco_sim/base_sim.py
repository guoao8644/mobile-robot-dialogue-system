...
class DefaultEnv:
    ...
    def cycle_camera(self):
        ...
        # モデル内のカメラ名リストを取得
        cam_names = ["(tracking)"] + [
            self.mj_model.camera(i).name for i in range(self.mj_model.ncam)
        ]
        self._camera_index = (self._camera_index + 1) % len(cam_names)
        name = cam_names[self._camera_index]
        if name == "(tracking)":
            self.viewer.cam.type = mujoco.mjtCamera.mjCAMERA_TRACKING
            self.viewer.cam.trackbodyid = self.mj_model.body("pelvis").id
            print(f"[Camera] tracking (pelvis)")
        else:
    ...
    def toggle_user_facing(self):
        ...
        mid = self.mj_model.body_mocapid[user_id]
        if mid < 0: return
        self._user_facing_g1 = not self._user_facing_g1
        if self._user_facing_g1:
            self.mj_data.mocap_quat[mid] = [0, 0, 0, 1]
            print("[User] G1 を向いている")
        else:
            self.mj_data.mocap_quat[mid] = [1, 0, 0, 0]
            print("[User] G1 に背を向けている")
