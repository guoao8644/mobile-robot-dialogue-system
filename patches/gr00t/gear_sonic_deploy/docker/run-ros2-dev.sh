...
TENSORRT_MOUNT=""
ADDITIONAL_MOUNTS=""
# deploy_entrypoint.sh をマウント（REPO_ROOT から探す）
_SCRIPT_REAL="$(readlink -f "$0")"
_REPO_ROOT="$(cd "$(dirname "$_SCRIPT_REAL")/../../../../.." && pwd)"
if [ -f "$_REPO_ROOT/scripts/deploy_entrypoint.sh" ]; then
    ADDITIONAL_MOUNTS="$ADDITIONAL_MOUNTS -v $_REPO_ROOT/scripts/deploy_entrypoint.sh:/deploy_entrypoint.sh:ro"
fi
DEVICE_MOUNTS=""
...
docker run -it --rm \
    ...
    bash -c "
        ...
        if [ -f "/deploy_entrypoint.sh" ]; then
            exec bash /deploy_entrypoint.sh
        else
            exec bash
        fi
    "
